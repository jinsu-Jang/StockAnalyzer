...
class OrderList(Resource):
    def get(self):
        ...
        if status == 'all':
            result_list = list(mongodb.find_items({}, "stocklab", "order"))
        elif status in ["buy_ordered", "buy_completed", "sell_ordered", "sell_completed"]:
            result_list = list(mongodb.find_items({"status":status}, "stocklab", "order"))
        else:
            return {}, 404
...
class UserList(Resource):
    def get(self):
        status = request.args.get('status', default="all", type=str)
        if status == 'all':
            result_list = list(mongodb.find_items({}, "stocklab", "user"))
        elif status in ["buy_ordered", "buy_completed", "sell_ordered", "sell_completed"]:
